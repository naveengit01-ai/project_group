...
const BASE_URL = "https://back-end-project-group.onrender.com"; // ✅ Fixed - was localhost:5000
...
const style = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header { text-align:center; margin-bottom:36px; animation:fadeUp 0.7s 0.1s cubic-bezier(0.16,1,0.3,1) both; }
  ...
  .subtitle { margin-top:6px; font-size:13px; color:var(--muted); font-family:'DM Mono',monospace; font-weight:300; }
  ...
  .input-row { position:relative; animation:slideIn 0.4s cubic-bezier(0.16,1,0.3,1) both; }
  ...
  .task-num {
    ...
    font-weight:500; pointer-events:none; opacity:0.7;
  }
  ...
  .add-btn {
    ...
    cursor:pointer; padding:6px 0; opacity:0.8;
  ...
  @keyframes taskIn { from{opacity:0;transform:translateY(16px) scale(0.98)} to{opacity:1;transform:translateY(0) scale(1)} }

  .task-item::before {
    content:''; position:absolute; left:0; top:0; bottom:0; width:3px;
    background:linear-gradient(180deg,var(--accent),var(--accent2));
    border-radius:3px 0 0 3px; opacity:0; transition:opacity 0.3s;
  }
  ...
  .task-item.done::before { opacity:1; background:var(--green); }
  ...
  .custom-check {
    position:absolute; inset:0;
    border:2px solid rgba(124,109,250,0.4); border-radius:7px;
  ...
  .task-item.done .custom-check {
    background:rgba(109,250,204,0.15); border-color:var(--green);
    box-shadow:0 0 12px rgba(109,250,204,0.3); transform:scale(1.1);
  }
  ...
  .task-item.done .check-icon { opacity:1; transform:scale(1) rotate(0deg); }
  ...
  .task-status.pending {
    background:rgba(124,109,250,0.1); color:rgba(124,109,250,0.8);
    border:1px solid rgba(124,109,250,0.2);
  }

  .task-status.complete {
    background:rgba(109,250,204,0.1); color:var(--green);
  ...
  .wait-notice {
    ...
    padding:12px 16px;
    background:rgba(250,219,109,0.07); border:1px solid rgba(250,219,109,0.2);
    border-radius:12px; color:var(--yellow);
    font-family:'DM Mono',monospace; font-size:12px;
    animation:fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) both;
  }

  .wait-icon { font-size:16px; animation:tdSpin 3s linear infinite; }
  @keyframes tdSpin { from{transform:rotate(0)} to{transform:rotate(360deg)} }

  /* PROGRESS */
  .progress-wrap { margin-bottom:24px; animation:fadeUp 0.6s 0.2s cubic-bezier(0.16,1,0.3,1) both; }
  ...
  .progress-count { font-family:'DM Mono',monospace; font-size:13px; font-weight:500; color:var(--accent); }

  .progress-track {
    height:4px; background:var(--surface2); border-radius:100px;
  ...
  .modal {
    ...
    border-radius:28px 28px 0 0; padding:36px 32px 40px;
  ...
  .modal-pill { width:36px; height:4px; background:rgba(255,255,255,0.15); border-radius:100px; margin:0 auto 28px; }

  .modal-icon {
    text-align:center; font-size:40px; margin-bottom:12px;
    animation:tdPop 0.5s 0.2s cubic-bezier(0.34,1.56,0.64,1) both;
  }

  @keyframes tdPop { from{opacity:0;transform:scale(0.5)} to{opacity:1;transform:scale(1)} }

  .modal-title { text-align:center; font-size:20px; font-weight:800; color:var(--text); margin-bottom:6px; letter-spacing:-0.01em; }

  .modal-sub { text-align:center; font-family:'DM Mono',monospace; font-size:12px; color:var(--muted); margin-bottom:28px; }
  ...
  .btn-yes:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 12px 32px rgba(124,109,250,0.55); }
  .btn-yes:disabled { opacity:0.6; cursor:not-allowed; }

  .btn-no {
    flex:1; padding:15px;
    background:rgba(250,109,154,0.08); border:1px solid rgba(250,109,154,0.25);
    border-radius:14px; color:var(--accent2);
    font-family:'Syne',sans-serif; font-size:15px; font-weight:700;
    cursor:pointer; transition:all 0.2s;
  }

  .btn-no:hover { background:rgba(250,109,154,0.15); border-color:rgba(250,109,154,0.4); transform:translateY(-2px); }
  ...
  .spinner {
    ...
    border-radius:50%; animation:tdSpin 0.7s linear infinite;
    vertical-align:middle; margin-right:8px;
  }

  .all-done { text-align:center; padding:16px; animation:fadeUp 0.5s cubic-bezier(0.16,1,0.3,1) both; }
  ...
  .all-done-sub { font-family:'DM Mono',monospace; font-size:12px; color:var(--muted); }
`;
...
export default function TO_DO_LIST() {
  ...
  const fetchTodayTodo = async () => {
    ...
    } catch { console.error("Fetch todo failed"); }
  };
  ...
  const handleSave = async () => {
    ...
    } catch { alert("Failed to save tasks"); }
  ...
  const markTask = async (taskId) => {
    try {
      ...
      const data = await res.json();
      if (data.status === "task_marked") { fetchTodayTodo(); setWaitMinutes(null); }
      if (data.status === "wait")         { setWaitMinutes(data.remainingMinutes); }
    } catch { alert("Failed to mark task"); }
  };
        ...
        <div className="card">
          ...
          {todoCreated && (
            <div className="task-list">
              {tasks.length === 0 && <div className="empty-state">No tasks for today yet.</div>}
              ...
              {tasks.map((task, i) => (
                ...
                >
                  ...
                  <div className="checkbox-wrap">
                    <input type="checkbox" checked={task.completed} disabled={task.completed || !!waitMinutes} onChange={()=>{}}/>
          ...
          <div className="overlay" onClick={() => setShowConfirm(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-actions">
                ...
                <button className="btn-no" onClick={() => setShowConfirm(false)}>Cancel</button>
              </div>
            </div>
          </div>
        )}
